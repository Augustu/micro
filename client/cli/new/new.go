...
import (
	...
	tmpl "github.com/Augustu/micro/v2/internal/template"
	"github.com/Augustu/micro/v2/internal/usage"
...
func protoComments(goDir, alias string) []string {
	return []string{
		...
		"go get github.com/Augustu/micro/v2/cmd/protoc-gen-micro",
...
type config struct {
	...
	FQDN string
	// github.com/Augustu/foo
	Dir string
	// $GOPATH/src/github.com/Augustu/foo
	GoDir string
	// $GOPATH
	GoPath string
	// UseGoPath
...
func copyAPIProto(c config) (string, error) {
	...
	if err != nil {
		return "", errors.New("Unable to find go-micro version. Please try `go get github.com/Augustu/go-micro/v2`")
	}
	newestDir := ""
	...
	input, err := ioutil.ReadFile(fmt.Sprintf("%s/pkg/mod/github.com/Augustu/go-micro/%s/api/proto/api.proto", basedir, newestDir))
	if err != nil {
		return "", err
	}
